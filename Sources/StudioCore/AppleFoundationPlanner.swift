...
public struct GeneratedEffectDraft: Equatable, Sendable {
    ...
    public init(type: String, strength: Double = 0.5, intensity: Double = 0.5, rate: Double = 1, duration: Double = 0.2, preset: String = "", text: String = "", target: String = "primary_person") {
        self.type = type
...
public struct GeneratedPlanMapper: Sendable {
    ...
    public func map(_ draft: GeneratedPlanDraft, for request: PlanningRequest) throws -> [EffectGraph] {
        guard draft.variants.count == request.variantCount else {
            throw GeneratedPlanMappingError.wrongVariantCount(expected: request.variantCount, actual: draft.variants.count)
        }
        let provenance = PlannerProvenance(kind: .localModel, name: "apple-foundation-model", version: "system")
        return try draft.variants.enumerated().map { variantIndex, variant in
            ...
            guard !variant.effects.isEmpty else { throw GeneratedPlanMappingError.emptyEffects(variantIndex) }
            ...
            return EffectGraph(
                ...
                timeline: [TimelineSegment(id: stableUUID(variant: variantIndex, segment: 0), start: 0, end: request.duration, effects: effects)]
            )
        }
    }
    ...
    private func parameters(for type: EffectType, draft: GeneratedEffectDraft) -> EffectParameters {
        ...
        case .backgroundReplace: return .init(preset: draft.preset.isEmpty ? "soft_gradient" : draft.preset)
        case .captionDynamic: return .init(preset: draft.preset.isEmpty ? "bold" : draft.preset, text: draft.text.isEmpty ? nil : draft.text)
        case .titleCard: return .init(text: draft.text.isEmpty ? "WATCH THIS" : draft.text)
        case .cropAutoSubject: return .init(target: draft.target.isEmpty ? "primary_person" : draft.target)
        default: return .init()
        }
    }
...
public struct PreferredVariantPlanner: VariantPlanning {
    ...
    public static var availability: LocalModelAvailability {
        ...
        if #available(macOS 26.0, *) {
            ...
            case .unavailable(.deviceNotEligible): return .unavailable("This Mac is not eligible for Apple Intelligence.")
            case .unavailable(.appleIntelligenceNotEnabled): return .unavailable("Apple Intelligence is not enabled in System Settings.")
            case .unavailable(.modelNotReady): return .unavailable("Apple’s on-device model is not ready yet.")
            @unknown default: return .unavailable("Apple’s on-device model is unavailable for an unknown system reason.")
            }
        }
#endif
    ...
    public func plan(_ request: PlanningRequest) async throws -> [EffectGraph] {
        if let primaryOverride {
            do { return try await primaryOverride(request) }
            catch { return try await fallback.plan(request) }
        }
#if canImport(FoundationModels)
...
private struct AppleGeneratedPlan {
    @Guide(description: "Create exactly the number of requested variants, from 2 through 5.", .count(2...5))
...
private struct AppleGeneratedVariant {
    ...
    var summary: String
    @Guide(description: "Use only supported effect identifiers supplied in the prompt.", .count(1...8))
...
private struct AppleFoundationPlanGenerator {
    func plan(_ request: PlanningRequest) async throws -> [EffectGraph] {
        ...
        let draft = GeneratedPlanDraft(variants: response.content.variants.map { variant in
            GeneratedVariantDraft(label: variant.label, summary: variant.summary, effects: variant.effects.map { effect in
                GeneratedEffectDraft(type: effect.type, strength: effect.strength, intensity: effect.intensity, rate: effect.rate, duration: effect.duration, preset: effect.preset, text: effect.text, target: effect.target)
            })
        })
