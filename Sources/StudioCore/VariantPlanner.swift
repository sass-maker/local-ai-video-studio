...
public struct DeterministicDemoPlanner: VariantPlanning {
    private let provenance = PlannerProvenance(kind: .deterministicDemo, name: "milestone-preset-planner", version: "1.0.0")
    ...
    public func plan(_ request: PlanningRequest) async throws -> [EffectGraph] {
        ...
        guard request.duration.isFinite, request.duration > 0 else { throw PlanningError.invalidDuration }
    ...
    private func baseEffects() -> [EffectNode] {
        [
            EffectNode(type: .cropAutoSubject, parameters: .init(target: "primary_person"), required: true),
            EffectNode(type: .audioNormalize, required: false),
        ]
    }

    private func animeRecipe(_ index: Int, _ request: PlanningRequest) -> EffectGraph {
        graph(index: index, label: "Cel Study", summary: "Cel-shaded approximation with restrained outlines and bold captions.", request: request, effects: baseEffects() + [
    ...
    private func comicRecipe(_ index: Int, _ request: PlanningRequest) -> EffectGraph {
        graph(index: index, label: "Comic Ink", summary: "Higher-contrast comic approximation with ink outlines and faster beat zooms.", request: request, effects: baseEffects() + [
    ...
    private func cleanPulseRecipe(_ index: Int, _ request: PlanningRequest) -> EffectGraph {
        graph(index: index, label: "Clean Pulse", summary: "Original subject with replaced background, dynamic captions, and restrained beat flashes.", request: request, effects: baseEffects() + [
    ...
    private func noirRecipe(_ index: Int, _ request: PlanningRequest) -> EffectGraph {
        graph(index: index, label: "Noir Cut", summary: "Monochrome cinematic approximation with slower crossfades.", request: request, effects: baseEffects() + [
    ...
    private func vhsRecipe(_ index: Int, _ request: PlanningRequest) -> EffectGraph {
        graph(index: index, label: "Tape Hook", summary: "VHS approximation with a short title hook and beat flashes.", request: request, effects: baseEffects() + [
    ...
    private func graph(index: Int, label: String, summary: String, request: PlanningRequest, effects: [EffectNode]) -> EffectGraph {
