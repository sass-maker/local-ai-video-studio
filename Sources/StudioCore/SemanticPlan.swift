...
public struct SemanticEffect: Codable, Equatable, Sendable {
  ...
  enum CodingKeys: String, CodingKey, StrictCodingKey {
    ...
    static let allKeys = [
      "type", "strength", "intensity", "rate", "duration", "target", "preset", "text",
    ]
  }
...
public struct SemanticPlan: Codable, Equatable, Sendable {
  ...
  public init(
    recordID: String, decision: SemanticPlanDecision, reason: String, variants: [SemanticVariant]
  ) {
...
public struct SemanticPlanCompiler: Sendable {
  ...
  public func compile(
    ...
    guard plan.decision == .plan else {
      guard plan.variants.isEmpty else {
        throw SemanticPlanCompilationError.nonPlanContainsVariants
      }
    ...
    guard plan.variants.count == request.variantCount else {
      throw SemanticPlanCompilationError.wrongVariantCount(
        expected: request.variantCount, actual: plan.variants.count)
    }
    ...
    let graphs = try plan.variants.enumerated().map { variantIndex, variant in
      ...
      guard !variant.effects.isEmpty else {
        throw SemanticPlanCompilationError.emptyEffects(variantIndex)
      }
        ...
        timeline: [
          TimelineSegment(
            ...
            effects: variant.effects.enumerated().map { effectIndex, effect in
              EffectNode(
                id: stableUUID(
                  seed: "\(plan.recordID):variant:\(variantIndex):effect:\(effectIndex)"),
                type: effect.type,
  ...
  private func outputProfile(for intent: SemanticOutputIntent, source: OutputProfile)
    -> OutputProfile
  {
    switch intent {
    case .vertical:
      OutputProfile(aspectRatio: .vertical, width: 1080, height: 1920, fps: source.fps)
    case .landscape:
      OutputProfile(aspectRatio: .landscape, width: 1920, height: 1080, fps: source.fps)
  ...
  private func stableUUID(seed: String) -> UUID {
    ...
    let value =
      "\(digest.prefix(8))-\(digest.dropFirst(8).prefix(4))-4\(digest.dropFirst(13).prefix(3))-8\(digest.dropFirst(17).prefix(3))-\(digest.dropFirst(20).prefix(12))"
