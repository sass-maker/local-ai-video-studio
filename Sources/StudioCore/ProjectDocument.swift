...
public struct SourceReference: Codable, Equatable, Sendable {
  ...
  public init(
    displayName: String, relativePath: String?, originalPath: String, bookmark: Data?,
    fingerprint: String, fileSize: Int64, metadata: MediaMetadata
  ) {
...
public struct RenderManifest: Codable, Equatable, Sendable {
  ...
  public init(
    state: RenderState, outputRelativePath: String? = nil, normalizedGraphHash: String,
    sourceFingerprint: String, degradations: [DegradationRecord] = [], completedAt: Date? = nil
  ) {
    self.state = state
...
public struct VariantRevision: Codable, Equatable, Identifiable, Sendable {
  ...
  public init(
    id: UUID = UUID(), revision: Int, graph: EffectGraph, normalizedGraphHash: String,
    render: RenderManifest? = nil
  ) {
    self.id = id
...
public struct VariantComparison: Codable, Equatable, Sendable {
  ...
  public init(
    ratings: [UUID: Int] = [:], selectedVariantID: UUID? = nil, isBlinded: Bool = false,
    sharedPlayhead: Double = 0
  ) {
...
public enum SourceLocator {
  public static func relativePath(from projectURL: URL, to sourceURL: URL) -> String? {
    let projectComponents = projectURL.deletingLastPathComponent().standardizedFileURL
      .pathComponents
    let sourceComponents = sourceURL.standardizedFileURL.pathComponents
    var common = 0
    while common < min(projectComponents.count, sourceComponents.count),
      projectComponents[common] == sourceComponents[common]
    {
      common += 1
    }
  ...
  public static func resolve(_ reference: SourceReference, relativeTo projectURL: URL) -> URL? {
    if let relativePath = reference.relativePath {
      let candidate = projectURL.deletingLastPathComponent().appending(path: relativePath)
        .standardizedFileURL
