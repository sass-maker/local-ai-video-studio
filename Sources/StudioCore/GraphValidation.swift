...
public struct EffectGraphValidator: Sendable {
  ...
  public func validate(_ input: EffectGraph) throws -> NormalizedGraph {
    ...
    if graph.schemaVersion != StudioCore.effectGraphSchemaVersion {
      diagnostics.append(
        .init(
          severity: .error, path: "schema_version",
          message: "Unsupported effect graph schema \(graph.schemaVersion)."))
    }
    if graph.label.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
      diagnostics.append(
        .init(severity: .error, path: "label", message: "Variant label cannot be empty."))
    }
    if graph.differenceSummary.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
      diagnostics.append(
        .init(
          severity: .error, path: "difference_summary",
          message: "A visible difference summary is required."))
    }
    ...
    for segmentIndex in graph.timeline.indices {
      ...
      if !segment.start.isFinite || !segment.end.isFinite || segment.start < 0
        || segment.end <= segment.start
      {
        diagnostics.append(
          .init(
            ...
            message:
              "Timeline interval must have finite, non-negative start and end greater than start."))
      }
      ...
      resolveBackgroundConflict(
        in: &graph.timeline[segmentIndex], path: path, diagnostics: &diagnostics)
    }
  ...
  private func validateOutput(_ output: OutputProfile, diagnostics: inout [GraphDiagnostic]) {
    if output.width < 64 || output.height < 64 || output.width > 7680 || output.height > 7680 {
      diagnostics.append(
        .init(
          severity: .error, path: "output",
          message: "Output dimensions must be between 64 and 7680 pixels."))
    }
    if !(1...120).contains(output.fps) {
      diagnostics.append(
        .init(
          severity: .error, path: "output.fps", message: "Frame rate must be between 1 and 120 FPS."
        ))
    }
    let isOrientationValid =
    ...
    if !isOrientationValid {
      diagnostics.append(
        .init(
          severity: .error, path: "output.aspect_ratio",
          message: "Dimensions do not match the declared aspect ratio."))
    }
  }
  ...
  private func validate(_ effect: EffectNode, path: String, diagnostics: inout [GraphDiagnostic]) {
    guard let definition = registry[effect.type] else {
      diagnostics.append(
        .init(severity: .error, path: "\(path).type", message: "Effect is not registered."))
      return
    }
    validate(
      effect.parameters.strength, rule: definition.strength, name: "strength", path: path,
      diagnostics: &diagnostics)
    validate(
      effect.parameters.intensity, rule: definition.intensity, name: "intensity", path: path,
      diagnostics: &diagnostics)
    validate(
      effect.parameters.rate, rule: definition.rate, name: "rate", path: path,
      diagnostics: &diagnostics)
    validate(
  ...
  private func validate(
    _ value: Double?, rule: ParameterRule?, name: String, path: String,
    diagnostics: inout [GraphDiagnostic]
  ) {
    ...
    guard let rule, value.isFinite, rule.contains(value) else {
      ...
      diagnostics.append(
        .init(
          severity: .error, path: "\(path).parameters.\(name)",
          message: "Parameter \(name) is outside its allowed range (\(range))."))
      return
    }
  }

  private func resolveBackgroundConflict(
    in segment: inout TimelineSegment, path: String, diagnostics: inout [GraphDiagnostic]
  ) {
    ...
    diagnostics.append(
      .init(
        severity: .warning, path: path,
        message: "Background replacement takes precedence; background blur was removed."))
  }
...
public struct RenderEstimator: Sendable {
  ...
  public func estimate(_ graph: EffectGraph) -> RenderEstimate {
    ...
    let responsible = Array(Set(types.filter { (registry[$0]?.cost ?? .heavy) >= .heavy })).sorted {
      $0.rawValue < $1.rawValue
    }
    let previewEdge = workClass == .heavy || !responsible.isEmpty ? 960 : 1280
    let estimatedBytes = Int64(
      max(duration, 1) * Double(graph.output.width * graph.output.height) * 0.12)
    return RenderEstimate(
      workClass: workClass, pixelFrames: pixelFrames, costUnits: costUnits,
      responsibleEffects: responsible, recommendedPreviewLongEdge: previewEdge,
