...
public struct GraphCompiler: Sendable {
  ...
  private func compile(_ node: EffectNode) throws -> CompiledOperation {
    ...
    case .backgroundBlur, .outline, .glow, .beatFlash, .beatZoom, .colorGrade:
      return compileFilterEffect(node.type, parameters: parameters)
    case .styleAnime, .styleComic, .styleSketch, .styleWatercolor, .styleCel,
      .styleCinematic, .styleNoir, .styleVHS:
      return compileStyleEffect(node.type, parameters: parameters)
    }
  }

  private func compileFilterEffect(_ type: EffectType, parameters: EffectParameters)
    -> CompiledOperation
  {
    switch type {
    case .backgroundBlur:
  ...
  private func compileStyleEffect(_ type: EffectType, parameters: EffectParameters)
    -> CompiledOperation
  {
    let strength = parameters.strength ?? 0.7
    switch type {
    case .styleAnime:
      return .filter(.anime, strength: strength)
    case .styleComic:
      return .filter(.comic, strength: strength)
    case .styleSketch:
      return .filter(.sketch, strength: strength)
    case .styleWatercolor:
      return .filter(.watercolor, strength: strength)
    case .styleCel:
      return .filter(.cel, strength: strength)
    case .styleCinematic:
      return .filter(.cinematic, strength: strength)
    case .styleNoir:
      return .filter(.noir, strength: strength)
    case .styleVHS:
      return .filter(.vhs, strength: strength)
    default:
      return .filter(.anime, strength: strength)
    }
  }
...
extension FilterRecipe {
  func applying(to input: CIImage, strength: Double, time: Double) -> CIImage {
    ...
    switch self {
    case .anime, .cel:
      let quantized = input.applyingFilter(
        "CIColorPosterize", parameters: ["inputLevels": 5 + amount * 3])
      let edges = input.applyingFilter(
        "CIEdges", parameters: [kCIInputIntensityKey: 1.2 + amount * 2]
      )
      .applyingFilter("CIColorInvert")
      return edges.applyingFilter(
        "CIMultiplyCompositing", parameters: [kCIInputBackgroundImageKey: quantized])
    case .comic:
      return input.applyingFilter("CIComicEffect")
    case .sketch:
      return input.applyingFilter(
        "CILineOverlay", parameters: ["inputNRNoiseLevel": 0.05, "inputEdgeIntensity": 1 + amount])
    case .watercolor:
      return input.applyingFilter("CIMedianFilter").applyingFilter(
        "CIColorControls", parameters: [kCIInputSaturationKey: 1.1 + amount * 0.3])
    case .cinematic:
      return input.applyingFilter(
        "CIColorControls",
        parameters: [
          kCIInputContrastKey: 1.05 + amount * 0.25, kCIInputSaturationKey: 0.9 + amount * 0.15,
        ])
    case .noir:
      return input.applyingFilter("CIPhotoEffectNoir")
    case .vhs:
      let shifted = input.transformed(
        by: CGAffineTransform(translationX: sin(time * 18) * amount * 4, y: 0))
      return shifted.applyingFilter(
        "CIColorControls", parameters: [kCIInputSaturationKey: 0.75, kCIInputContrastKey: 1.15])
    case .colorGrade:
      return input.applyingFilter(
        "CIColorControls",
        parameters: [
          kCIInputContrastKey: 1 + amount * 0.25, kCIInputSaturationKey: 1 + amount * 0.2,
        ])
    case .backgroundBlur:
      return input.applyingFilter("CIGaussianBlur", parameters: [kCIInputRadiusKey: amount * 18])
        .cropped(to: input.extent)
    case .outline:
      let edges = input.applyingFilter(
        "CIEdges", parameters: [kCIInputIntensityKey: 1 + amount * 3])
      return edges.applyingFilter(
        "CIScreenBlendMode", parameters: [kCIInputBackgroundImageKey: input])
    case .glow:
      return input.applyingFilter(
        "CIBloom", parameters: [kCIInputIntensityKey: amount, kCIInputRadiusKey: 4 + amount * 16])
    case .beatFlash:
    ...
    case .beatZoom:
      ...
      return input.transformed(
        by: CGAffineTransform(translationX: input.extent.midX, y: input.extent.midY)
      )
