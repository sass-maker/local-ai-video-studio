...
public struct MediaAnalyzer: Sendable {
  ...
  public func analyze(_ url: URL) async throws -> MediaMetadata {
    ...
    let size = try await video.load(.naturalSize).applying(
      try await video.load(.preferredTransform))
    let fps = try await video.load(.nominalFrameRate)
    let descriptions = try await video.load(.formatDescriptions)
    let codec =
      descriptions.first.map { description in
  ...
  public func makeSourceReference(for url: URL, projectURL: URL?, metadata: MediaMetadata) throws
    -> SourceReference
  {
