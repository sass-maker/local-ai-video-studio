...
public struct StudioAgentService: Sendable {
  ...
  public func run(_ raw: Any) async throws -> sending [String: Any] {
    ...
    case "plan":
      try exact(
        request.input,
        allowed: [
          "instruction", "variantCount", "durationSeconds", "aspectRatio", "width", "height", "fps",
        ],
        path: "input"
      )
      ...
      guard let aspect = AspectRatio(rawValue: aspectRaw) else {
        throw StudioAgentError(
          "INVALID_INPUT", "input.aspectRatio is unsupported", path: "input.aspectRatio")
      }
    ...
    case "edit":
      ...
      do {
        ...
        case "trim-segment":
          try exact(
            request.input, allowed: ["graph", "action", "segmentId", "start", "end"], path: "input")
    ...
    case "select":
      ...
      guard project.variants.contains(where: { $0.graph.id == id }) else {
        throw StudioAgentError(
          "VARIANT_NOT_FOUND", "variant is not present in project", path: "input.variantId")
      }
    ...
    case "render":
      try exact(
        request.input, allowed: ["sourcePath", "outputPath", "graph", "sourceFingerprint", "mode"],
        path: "input")
      ...
      if request.validateOnly {
        ...
        let mode: RenderRequest.Mode =
          request.input["mode"] as? String == "final" ? .final : .preview
        let manifest = try await renderer.render(
          .init(
            sourceURL: source, outputURL: output, normalizedGraph: normalized,
            sourceFingerprint: fingerprint, mode: mode))
    ...
    case "cancel":
      ...
      throw StudioAgentError(
        "NO_ACTIVE_OPERATION",
        "studio-agent runs one foreground operation per process; stop the process to cancel")
    case "export":
      ...
      guard let selected = project.comparison.selectedVariantID,
        ...
        throw StudioAgentError(
          "EXPORT_NOT_READY", "selected variant has no current completed render")
      }
    ...
    default:
      throw StudioAgentError(
        "UNKNOWN_OPERATION", "unknown Studio operation: \(operation)", path: "operation")
    }
  ...
  private func manifest() -> [String: Any] {
    [
      ...
      "safety": [
        "arbitraryExecution": false, "localByDefault": true, "externalPublication": false,
      ],
  ...
  private func normalize(_ raw: Any) throws -> Request {
    guard let root = raw as? [String: Any] else {
      throw StudioAgentError("INVALID_REQUEST", "request must be a JSON object")
    }
    try exact(root, allowed: Self.rootFields, path: "request")
    guard root["schema"] as? String == studioAgentSchema else {
      throw StudioAgentError(
        "UNSUPPORTED_SCHEMA", "schema must be \(studioAgentSchema)", path: "schema")
    }
    guard root["product"] as? String == "studio" else {
      throw StudioAgentError("PRODUCT_MISMATCH", "product must be studio", path: "product")
    }
    ...
    return Request(
      operation: operation, operationID: root["operationId"] as? String ?? UUID().uuidString,
      idempotencyKey: root["idempotencyKey"] as? String,
      validateOnly: root["validateOnly"] as? Bool == true, input: input)
  }
  ...
  private func exact(_ object: [String: Any], allowed: Set<String>, path: String) throws {
    if let key = object.keys.first(where: { !allowed.contains($0) }) {
      throw StudioAgentError(
        "UNKNOWN_FIELD", "unknown field: \(path).\(key)", path: "\(path).\(key)")
    }
  }
  ...
  private func rejectForbidden(_ value: Any, path: String) throws {
    if let object = value as? [String: Any] {
      for (key, nested) in object {
        if Self.forbiddenFields.contains(key) {
          throw StudioAgentError(
            "ARBITRARY_EXECUTION_REJECTED", "\(path).\(key) is not accepted", path: "\(path).\(key)"
          )
        }
      ...
      for (index, nested) in array.enumerated() {
        try rejectForbidden(nested, path: "\(path)[\(index)]")
      }
    }
  }
  ...
  private func graph(from value: Any?) throws -> EffectGraph {
    guard let value else {
      throw StudioAgentError("REQUIRED_FIELD", "input.graph is required", path: "input.graph")
    }
    return try JSONDecoder().decode(
      EffectGraph.self, from: JSONSerialization.data(withJSONObject: value))
  }

  private func normalizedObject(_ value: NormalizedGraph) throws -> [String: Any] {
    [
      "graph": try object(value.graph), "graphHash": value.canonicalHash,
      "warnings": try value.warnings.map(object),
    ]
  }
  ...
  private func requiredString(_ object: [String: Any], _ key: String, prefix: String = "input.")
    throws -> String
  {
    guard let value = object[key] as? String,
      ...
      throw StudioAgentError(
        "REQUIRED_FIELD", "\(prefix)\(key) is required", path: "\(prefix)\(key)")
    }
    return value
  }

  private func optionalString(_ object: [String: Any], _ key: String, default defaultValue: String)
    throws -> String
  {
    ...
    guard let text = value as? String, !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ...
      throw StudioAgentError(
        "INVALID_INPUT", "input.\(key) must be a non-empty string", path: "input.\(key)")
    }
    return text
  }

  private func optionalInt(_ object: [String: Any], _ key: String, default defaultValue: Int) throws
    -> Int
  {
    guard let value = object[key] else { return defaultValue }
    guard let number = value as? NSNumber, CFGetTypeID(number) != CFBooleanGetTypeID(),
      Double(truncating: number).rounded() == Double(truncating: number)
    else {
      throw StudioAgentError(
        "INVALID_INPUT", "input.\(key) must be an integer", path: "input.\(key)")
    }
    return number.intValue
  }

  private func optionalDouble(_ object: [String: Any], _ key: String, default defaultValue: Double)
    throws -> Double
  {
    guard let value = object[key] else { return defaultValue }
  ...
  private func requiredDouble(_ object: [String: Any], _ key: String) throws -> Double {
    guard let value = object[key] as? NSNumber,
      ...
      throw StudioAgentError(
        "INVALID_INPUT", "input.\(key) must be a finite number", path: "input.\(key)")
    }
  ...
  private func requiredPath(_ object: [String: Any], _ key: String) throws -> URL {
    ...
    guard FileManager.default.fileExists(atPath: url.path) else {
      throw StudioAgentError("PATH_NOT_FOUND", "input.\(key) does not exist", path: "input.\(key)")
    }
    return url
  }
  ...
  private func requiredUUID(_ object: [String: Any], _ key: String) throws -> UUID {
    guard let value = UUID(uuidString: try requiredString(object, key)) else {
      throw StudioAgentError("INVALID_UUID", "input.\(key) must be a UUID", path: "input.\(key)")
    }
    return value
  }

  private func effectType(_ value: Any?) throws -> EffectType {
    guard let raw = value as? String, let type = EffectType(rawValue: raw) else {
      throw StudioAgentError(
        "UNSUPPORTED_EFFECT", "input.effect must name a registered effect", path: "input.effect")
    }
    return type
  }
