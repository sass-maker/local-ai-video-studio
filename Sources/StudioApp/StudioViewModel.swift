...
final class StudioViewModel: ObservableObject {
  ...
  @Published var instruction =
    "Create three vertical reels. Use cel-shaded anime, comic-book styling, and an original treatment with a replaced background, captions, and beat-synced flashes."
  ...
  var diskPreflight: DiskSpacePreflight? {
    ...
    return DiskSpacePreflight(
      requiredBytes: estimate.estimatedBytes * Int64(max(variants.count, 1)),
  ...
  func importVideo(_ url: URL) {
    ...
    Task {
      ...
      do {
        ...
        let reference = try analyzer.makeSourceReference(
          for: url, projectURL: nil, metadata: metadata)
        self.sourceURL = url
  ...
  private func renderVariants(targetIDs: Set<UUID>?) {
    ...
    renderTask = Task {
      let outputDirectory = FileManager.default.temporaryDirectory
        .appending(
          path: "LocalVideoStudio-\(source.fingerprint.prefix(12))", directoryHint: .isDirectory)
      do {
        ...
        for variant in variants where targetIDs?.contains(variant.graph.id) ?? true {
          ...
          do {
            ...
            diagnostics.append(
              .init(
                severity: .error, path: variant.graph.label, message: error.localizedDescription))
          }
        }
        guard self.renderOperationID == operationID else { return }
        message =
  ...
  var planJSON: String {
    ...
    guard let data = try? encoder.encode(variants.map(\.graph)) else {
  ...
  func reviseStrength(for id: UUID, strength: Double) {
    ...
    do {
      ...
      states[id] = .planned
      message =
        "Created a new plan revision. The previous preview remains available until rerender succeeds."
      persistProject()
    } catch let failure as GraphValidationFailure {
      diagnostics = failure.diagnostics
    } catch {
  ...
  func updateEffect(_ type: EffectType, parameter: EffectTextParameterKind, value: String) {
    ...
    do {
      variants[index] = try graphEditor.updating(
        type: type, parameter: parameter, value: value, in: variants[index].graph)
      states[selectedID] = .planned
      diagnostics = variants.flatMap(\.warnings)
      message = "Updated \(parameter.rawValue). The previous preview is stale until rerendered."
      persistProject()
  ...
  private func editGraphs(
    ...
    let targetIDs =
      toAllVariants ? Set(variants.map { $0.graph.id }) : Set([selectedID].compactMap { $0 })
    ...
    do {
      ...
      diagnostics = variants.flatMap(\.warnings)
      message =
        "\(action) \(toAllVariants ? "to all variants" : "to the selected variant"). Previous previews are stale until rerendered."
      persistProject()
    } catch let failure as GraphValidationFailure {
      diagnostics = failure.diagnostics
  ...
  private func makeProjectURL(for source: SourceReference) -> URL? {
    ...
    let directory = applicationSupport.appending(
      path: "LocalVideoStudio/Projects", directoryHint: .isDirectory)
    do {
  ...
  private func persistProject() {
    ...
    let project = StudioProject(
      id: projectID,
      name: source.displayName.replacingOccurrences(
        of: ".\((source.displayName as NSString).pathExtension)", with: ""),
    ...
    saveTask = Task {
      ...
      do { try await projectStore.save(project, to: projectURL) } catch {
        self.message = "Project save failed: \(error.localizedDescription)"
      }
    }
  }
  ...
  private func writeComparisonSheet(to directory: URL) throws {
    ...
    try encoder.encode(rows).write(
      to: directory.appending(path: "comparison-sheet.json"), options: .atomic)

    var markdown =
      "# Variant comparison\n\n| Variant | Style/effects | Duration | Ratio | Saved | Selected | State |\n|---|---|---:|---|---|---|---|\n"
    for row in rows {
      markdown +=
        "| \(row.label) | \(row.effects.joined(separator: ", ")) | \(String(format: "%.1fs", row.duration)) | \(row.aspectRatio) | \(row.savedForLater ? "Yes" : "No") | \(row.selected ? "Yes" : "No") | \(row.state) |\n"
    }
    try Data(markdown.utf8).write(
      to: directory.appending(path: "comparison-sheet.md"), options: .atomic)
  }
}
