...
struct StudioWorkspace: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      GeometryReader { proxy in
        HStack(spacing: 0) {
          ...
          comparisonWorkspace(
            availableWidth: proxy.size.width - (proxy.size.width >= 980 ? 269 : 0))
        }
      }
      statusBar
    }
    ...
    .confirmationDialog(
      "Replace source and clear this comparison?", isPresented: $confirmingSourceReplacement
    ) {
  ...
  private var fullTopBar: some View {
    HStack(spacing: 18) {
      ...
      if let estimate = model.estimate {
        Text(
          "\(estimate.workClass.rawValue.capitalized) work · \(estimate.recommendedPreviewLongEdge)p edge"
        )
      ...
      if let disk = model.diskPreflight, !disk.canStart {
        Label(
          "Needs \(ByteCountFormatter.string(fromByteCount: disk.additionalBytesRequired, countStyle: .file)) more",
      ...
      Button {
        ...
        Label(
          model.isBlinded ? "Reveal Labels" : "Blind Labels",
          systemImage: model.isBlinded ? "eye" : "eye.slash")
      }
    ...
    Menu {
      ForEach(StudioAppearance.allCases) { appearance in
        Button {
          ...
          Label(
            appearance.label,
            systemImage: appearance == selectedAppearance ? "checkmark" : appearance.symbol)
        }
      }
    } label: {
  ...
  private func comparisonWorkspace(availableWidth: CGFloat) -> some View {
    VStack(spacing: 0) {
      if model.variants.isEmpty {
        ...
        DifferenceStrip(
          variants: model.variants, playhead: model.sharedTime, blinded: model.isBlinded
        )
        .frame(height: 230)
      ...
      HStack(spacing: 8) {
        ForEach(Array(model.variants.enumerated()), id: \.element.graph.id) { index, variant in
          Button("\(index + 1)") { model.select(variant.graph.id) }
            ...
            .tint(
              variant.graph.id == model.selectedID ? StudioPalette.probe : StudioPalette.mutedInk
            )
            .accessibilityLabel("Show study \(index + 1)")
        }
        Spacer()
        Text("ONE-STUDY REVIEW").font(.caption.weight(.bold)).foregroundStyle(
          StudioPalette.mutedInk)
      }
      .padding(.horizontal, 18)
      .padding(.top, 12)
      if let index = model.variants.firstIndex(where: { $0.graph.id == model.selectedID })
        ?? model.variants.indices.first
      {
  ...
  private var statusBar: some View {
    HStack(spacing: 10) {
      ...
      if let source = model.source {
        Text(
          "\(source.metadata.width)×\(source.metadata.height) · \(String(format: "%.2f", source.metadata.fps)) FPS · \(source.metadata.codec)"
        )
        .font(.caption.monospaced())
        .foregroundStyle(StudioPalette.mutedInk)
      }
...
private struct EffectCatalogRow: View {
      ...
      HStack(alignment: .firstTextBaseline) {
        ...
        if appliedNode == nil {
          Button("Add", action: onAdd)
            ...
            .accessibilityHint(
              applyToAll ? "Applies to every variant" : "Applies to the selected variant")
        } else {
      ...
      if let appliedNode {
        ForEach(definition.parameters, id: \.kind) { descriptor in
          EffectParameterControl(
            ...
            value: parameterValue(descriptor.kind, from: appliedNode.parameters)
        ...
        ForEach(definition.textParameters, id: \.kind) { descriptor in
          EffectTextParameterControl(
            descriptor: descriptor,
            effectName: definition.displayName,
            value: textParameterValue(descriptor.kind, from: appliedNode.parameters)
              ?? descriptor.defaultValue,
  ...
  private func parameterValue(_ kind: EffectParameterKind, from parameters: EffectParameters)
    -> Double?
  {
    switch kind {
  ...
  private func textParameterValue(
    _ kind: EffectTextParameterKind, from parameters: EffectParameters
  ) -> String? {
    switch kind {
...
private struct EffectTextParameterControl: View {
  ...
  init(
    descriptor: EffectTextParameterDescriptor, effectName: String, value: String,
...
private struct EffectParameterControl: View {
  ...
  init(
    descriptor: EffectParameterDescriptor, effectName: String, value: Double,
  ...
  var body: some View {
    HStack {
      Text(descriptor.kind.rawValue.capitalized).font(.caption)
      Slider(value: $draft, in: descriptor.rule.minimum...descriptor.rule.maximum, step: step) {
        editing in
  ...
  private var step: Double {
    ...
    case .strength, .intensity: 0.01
    case .rate, .duration: 0.05
    }
  }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      if let source = model.source {
        VStack(alignment: .leading, spacing: 7) {
          Label(source.displayName, systemImage: "film")
            ...
            .lineLimit(2)
          Text(
            "\(time(source.metadata.duration))  ·  \(source.metadata.width)×\(source.metadata.height)"
          )
          .font(.caption.monospaced())
          .foregroundStyle(StudioPalette.mutedInk)
        }
      ...
      Button(action: requestImport) {
        Label(
          model.source == nil ? "Import Video" : "Replace Source",
          systemImage: "square.and.arrow.down")
      }
      .buttonStyle(PrinterSecondaryButtonStyle())

      Divider().overlay(StudioPalette.hairline)
      Text("EDIT INSTRUCTION").font(.caption.weight(.semibold)).foregroundStyle(
        StudioPalette.mutedInk)
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 8) {
            ForEach(Array(model.diagnostics.enumerated()), id: \.offset) { _, item in
              Label(
                item.message,
                systemImage: item.severity == .error ? "xmark.octagon" : "exclamationmark.triangle"
              )
              .font(.caption)
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      VStack(alignment: .leading, spacing: 12) {
        ...
        if variant.graph.timeline.flatMap(\.effects).contains(where: {
          $0.parameters.strength != nil
        }) {
    ...
    .overlay(
      Rectangle().stroke(
        isSelected ? StudioPalette.probe : StudioPalette.hairline, lineWidth: isSelected ? 2 : 1)
    )
...
private struct PlanInspector: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      Text(
        "This is the exact validated graph produced from your instruction and compiled into local filters."
      )
      .font(.body)
...
private struct DifferenceStrip: View {
        ...
        ForEach(Array(variants.enumerated()), id: \.element.graph.id) { index, variant in
          VStack(alignment: .leading, spacing: 6) {
            Text(blinded ? "STUDY \(index + 1)" : variant.graph.label.uppercased())
              .font(.caption.weight(.bold))
            ForEach(Array(variant.graph.timeline.flatMap(\.effects).prefix(4)), id: \.id) {
              effect in
  ...
  private func effectLine(_ effect: EffectNode) -> String {
    ...
    return value.map { "\(effect.type.rawValue)  \(String(format: "%.2f", $0))" }
      ?? effect.type.rawValue
  }

  private func timecode(_ seconds: Double) -> String {
    String(
      format: "%02d:%02d:%02d", Int(seconds) / 3600, (Int(seconds) / 60) % 60, Int(seconds) % 60)
  }
}
...
private struct PlaybackRail: View {
  ...
  var body: some View {
    HStack(spacing: 14) {
      ...
      Slider(
        value: Binding(get: { model.sharedTime }, set: { model.seekAll(to: $0) }),
...
private enum StudioPalette {
  static let canvas = adaptive(
    light: NSColor(red: 0.91, green: 0.90, blue: 0.86, alpha: 1),
    dark: NSColor(red: 0.018, green: 0.022, blue: 0.022, alpha: 1))
  static let secondaryStock = adaptive(
    light: NSColor(red: 0.84, green: 0.82, blue: 0.77, alpha: 1),
    dark: NSColor(red: 0.035, green: 0.043, blue: 0.043, alpha: 1))
  static let paper = adaptive(
    light: NSColor(red: 0.96, green: 0.95, blue: 0.91, alpha: 1),
    dark: NSColor(red: 0.055, green: 0.065, blue: 0.065, alpha: 1))
  static let study = adaptive(
    light: NSColor(red: 0.95, green: 0.94, blue: 0.90, alpha: 1),
    dark: NSColor(red: 0.028, green: 0.035, blue: 0.035, alpha: 1))
  static let ink = adaptive(
    light: NSColor(red: 0.12, green: 0.15, blue: 0.16, alpha: 1),
    dark: NSColor(red: 0.91, green: 0.90, blue: 0.86, alpha: 1))
  static let mutedInk = adaptive(
    light: NSColor(red: 0.35, green: 0.38, blue: 0.38, alpha: 1),
    dark: NSColor(red: 0.68, green: 0.69, blue: 0.67, alpha: 1))
  static let hairline = adaptive(
    light: NSColor(red: 0.55, green: 0.53, blue: 0.50, alpha: 1),
    dark: NSColor(red: 0.18, green: 0.20, blue: 0.20, alpha: 1))
  static let probe = adaptive(
    light: NSColor(red: 0.58, green: 0.25, blue: 0.16, alpha: 1),
    dark: NSColor(red: 0.95, green: 0.60, blue: 0.45, alpha: 1))
  static let ready = adaptive(
    light: NSColor(red: 0.12, green: 0.40, blue: 0.36, alpha: 1),
    dark: NSColor(red: 0.42, green: 0.82, blue: 0.75, alpha: 1))
  static let warning = adaptive(
    light: NSColor(red: 0.49, green: 0.27, blue: 0.05, alpha: 1),
    dark: NSColor(red: 0.96, green: 0.70, blue: 0.36, alpha: 1))
