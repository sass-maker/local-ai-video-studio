...
private let semanticProvenance = PlannerProvenance(
  kind: .localModel, name: "test-teacher", version: "1")
...
@Test func semanticPlanRejectsUnsupportedParameterForEffect() throws {
  let plan = SemanticPlan(
    ...
    variants: [
      SemanticVariant(
        label: "A", output: .vertical, effects: [.init(type: .backgroundBlur, intensity: 0.5)]),
      SemanticVariant(
        label: "B", output: .vertical, effects: [.init(type: .backgroundBlur, intensity: 0.7)]),
    ]
  )

  #expect(throws: SemanticPlanCompilationError.self) {
    try SemanticPlanCompiler().compile(
      plan, request: semanticRequest, provenance: semanticProvenance)
  }
}

@Test func semanticPlanPreservesClarificationWithoutGraphs() throws {
  let plan = SemanticPlan(
    recordID: "clarify", decision: .clarify, reason: "Choose an aspect ratio", variants: [])
  let result = try SemanticPlanCompiler().compile(
    plan, request: semanticRequest, provenance: semanticProvenance)
...
@Test func finalDevinSemanticPilotCompilesEveryPlannedVariant() throws {
  ...
  for (index, plan) in plans.enumerated() {
    ...
    let result = try SemanticPlanCompiler().compile(
      plan, request: request, provenance: semanticProvenance)
