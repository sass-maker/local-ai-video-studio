...
private func graph(
  effects: [EffectNode] = [EffectNode(type: .styleCel, parameters: .init(strength: 0.7))]
) -> EffectGraph {
  EffectGraph(
    ...
    timeline: [
      TimelineSegment(
        id: UUID(uuidString: "00000000-0000-4000-8000-000000000002")!, start: 0, end: 4.5,
        effects: effects)
    ]
  )
}
...
@Test func validatorNormalizesTimelineAndProducesStableHash() throws {
  ...
  input.timeline = [
    TimelineSegment(
      id: UUID(uuidString: "00000000-0000-4000-8000-000000000004")!, start: 4, end: 5, effects: []),
    TimelineSegment(
      id: UUID(uuidString: "00000000-0000-4000-8000-000000000003")!, start: 0, end: 4, effects: []),
  ]
...
@Test func effectCatalogCoversEveryRegisteredEffectTruthfully() {
  ...
  #expect(
    catalog.filter { $0.readiness == .fallback }.allSatisfy { $0.fallbackReason?.isEmpty == false })
...
@Test func directEffectEditingUsesTheValidatedCanonicalGraph() throws {
  ...
  let flash = try #require(
    added.graph.timeline.flatMap(\.effects).first(where: { $0.type == .beatFlash }))
  #expect(added.canonicalHash != original.canonicalHash)
  #expect(flash.parameters.intensity == 0.5)

  let tuned = try editor.updating(
    type: .beatFlash, parameter: .intensity, value: 0.8, in: added.graph)
  #expect(
    tuned.graph.timeline.flatMap(\.effects).first(where: { $0.id == flash.id })?.parameters
      .intensity == 0.8)
...
@Test func directEditingRejectsOutOfRangeParameters() throws {
  ...
  let speed = try #require(
    added.graph.timeline.flatMap(\.effects).first(where: { $0.type == .speed }))
...
@Test func directEditingExposesTypedTitleAndPresetParameters() throws {
  ...
  let renamed = try editor.updating(
    type: .titleCard, parameter: .text, value: "Launch Day", in: titled.graph)
  #expect(
    renamed.graph.timeline.flatMap(\.effects).first(where: { $0.type == .titleCard })?.parameters
      .text == "Launch Day")

  let captioned = try editor.adding(.captionDynamic, to: renamed.graph)
  let tuned = try editor.updating(
    type: .captionDynamic, parameter: .preset, value: "karaoke", in: captioned.graph)
  #expect(
    tuned.graph.timeline.flatMap(\.effects).first(where: { $0.type == .captionDynamic })?.parameters
      .preset == "karaoke")
}
...
@Test func milestonePlannerCreatesMeaningfulValidatedVariants() async throws {
  let request = PlanningRequest(
    instruction:
      "Create three vertical reels: anime, comic, and original with background, captions, and beat flashes.",
    variantCount: 3,
    output: output,
    duration: 38
  )
...
@Test func generatedPlanMapsPromptSpecificEffectsAndModelProvenance() throws {
  let request = PlanningRequest(
    instruction: "watercolor, slow motion, and glow", variantCount: 2, output: output, duration: 12)
  let draft = GeneratedPlanDraft(variants: [
    .init(
      label: "Water Glow", summary: "Soft painted treatment",
      effects: [
        .init(type: EffectType.styleWatercolor.rawValue, strength: 0.8),
        .init(type: EffectType.glow.rawValue, strength: 0.35),
      ]),
    .init(
      label: "Slow Wash", summary: "Slower alternate",
      effects: [
...
@Test func generatedPlanRejectsUnknownEffectsBeforeValidation() {
  let request = PlanningRequest(
    instruction: "explode it", variantCount: 2, output: output, duration: 5)
...
@Test func generatedPlanParametersStillPassThroughExistingBoundsValidator() throws {
  let request = PlanningRequest(
    instruction: "too strong", variantCount: 2, output: output, duration: 5)
  let draft = GeneratedPlanDraft(variants: [
    .init(
      label: "A", summary: "A", effects: [.init(type: EffectType.styleComic.rawValue, strength: 2)]),
    .init(label: "B", summary: "B", effects: [.init(type: EffectType.styleNoir.rawValue)]),
  ])
  let graphs = try GeneratedPlanMapper().map(draft, for: request)
...
@Test func projectRoundTripsAndPreservesComparison() async throws {
  ...
  let source = SourceReference(
    ...
    metadata: MediaMetadata(
      duration: 5, width: 1920, height: 1080, fps: 24, hasAudio: true, codec: "avc1")
  )
  let normalized = try EffectGraphValidator().validate(graph())
  let revision = VariantRevision(
    revision: 1, graph: normalized.graph, normalizedGraphHash: normalized.canonicalHash)
  let project = StudioProject(
    ...
    comparison: VariantComparison(
      ratings: [normalized.graph.id: 4], selectedVariantID: normalized.graph.id, isBlinded: true,
      sharedPlayhead: 2.4)
  )
...
@Test func projectStoreRejectsUnsupportedSchema() async throws {
  ...
  let source = SourceReference(
    ...
    fileSize: 0,
    metadata: MediaMetadata(
      duration: 1, width: 1, height: 1, fps: 1, hasAudio: false, codec: "none")
  )
