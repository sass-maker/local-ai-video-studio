...
func applePlannerSemanticEvaluation() async throws {
    ...
    let cases: [PlannerEvaluationCase] = [
        .init(
            ...
            prompt: "Create two watercolor versions. One should be dreamy and slow motion; the other should be a lighter watercolor treatment.",
            variantCount: 2,
        ...
        .init(
            name: "noir-restraint",
            prompt: "Make three restrained noir edits with cinematic contrast. Do not use anime, comic, VHS, captions, or title cards.",
            variantCount: 3,
            expected: [.styleNoir],
        ...
        .init(
            name: "comic-subject",
            prompt: "Create three comic-book versions with a clear subject outline and a subtle glow. Keep the subject recognizable.",
            variantCount: 3,
        ...
        .init(
            ...
            prompt: "Create two cinematic versions with a softly blurred background and restrained color grading.",
            variantCount: 2,
        ...
        .init(
            name: "clean-original",
            prompt: "Create three clean versions. Keep the original visual style, normalize audio, and reframe around the primary person. No stylization.",
            variantCount: 3,
            expected: [.audioNormalize, .cropAutoSubject],
            forbidden: [.styleAnime, .styleComic, .styleSketch, .styleWatercolor, .styleCel, .styleCinematic, .styleNoir, .styleVHS]
        ),
        .init(
            name: "captions-and-tracking",
            prompt: "Create five vertical social versions with bold dynamic captions and automatic framing around the main person.",
            variantCount: 5,
        ...
        .init(
            ...
            prompt: "Turn the speaker into a photorealistic 3D dragon and replace the entire world. If that is unsupported, use only safe available effects and explain the approximation through the variant summary.",
            variantCount: 2,
            expected: [],
            forbidden: []
        ),
    ...
    for item in cases {
        ...
        print("APPLE_PLANNER_CASE name=\(item.name) apple=\(usedApple) expected=\(item.expected.map(\.rawValue).sorted()) found=\(found.map(\.rawValue).sorted()) forbidden=\(forbidden.map(\.rawValue).sorted()) effects=\(effects.map(\.rawValue).sorted())")
    }

    let recall = expectedTotal == 0 ? 1 : Double(covered) / Double(expectedTotal)
    print("APPLE_PLANNER_SUMMARY cases=\(cases.count) semantic_recall=\(String(format: "%.3f", recall)) forbidden_hits=\(forbiddenHits) fallback_cases=\(fallbackCases)")
}
