...
private func normalized(label: String, id: UUID, effects: [EffectNode]) throws -> NormalizedGraph {
    try EffectGraphValidator().validate(EffectGraph(
        id: id,
        label: label,
...
@Test func compilerPreservesTimelineAndEffectOrder() throws {
    let graph = try normalized(label: "Test", id: UUID(), effects: [
    ...
    #expect(pipeline.segments[0].operations == [
...
@Test func coordinatorIsolatesVariantFailureAndCompletesSiblings() async throws {
    ...
    let requests = graphs.map {
        RenderRequest(sourceURL: URL(filePath: "/source.mov"), outputURL: URL(filePath: "/\($0.graph.label).mp4"), normalizedGraph: $0, sourceFingerprint: "source")
    }
...
@Test func analyzerAndRendererProcessLocalFixture() async throws {
    ...
    let graph = try normalized(label: "Noir", id: UUID(), effects: [
        EffectNode(type: .styleNoir, parameters: .init(strength: 0.7)),
    ])
    let fingerprint = try SourceFingerprint.make(for: sourceURL)
    let manifest = try await AVFoundationRenderer().render(RenderRequest(
        sourceURL: sourceURL,
        outputURL: outputURL,
...
private func makeFixtureVideo(at url: URL) async throws {
    ...
    for frame in 0..<24 {
        ...
        #expect(adaptor.append(pixelBuffer, withPresentationTime: CMTime(value: CMTimeValue(frame), timescale: 24)))
    }
